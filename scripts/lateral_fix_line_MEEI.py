# -*- coding: utf-8 -*-
# @Time    : 2022-08-11 16:23
# @Author  : young wang
# @FileName: lateral_fix_line_MEEI.py
# @Software: PyCharm
...
if __name__ == '__main__':
    ...
    data_sets = natsorted(glob.glob('../data/MEEI/line grid/*.oct'))
    ...
    plt.show()
    #
    # temp_img = exposure.equalize_adapthist(ori_img, clip_limit=0.05)
    # temp_img = opening(temp_img, square(4))
    # img = median_filter(temp_img, 3)
    #
    # edge_img = feature.canny(temp_img, sigma=2.75)
    ...
    # nosie_le = 2
    #
    # line_img = lprep.convert_chessboard_to_linepattern(ori_img,bgr=False)
    # img_list.append(line_img)
    # title_list.append('binary line map & perspective feature lines')
    #
    # fig, ax = plt.subplots(1, 1, figsize=(16, 9), constrained_layout=True)
    # # for n, (ax, image, title) in enumerate(zip(axs.flat, img_list, title_list)):
    # ax.imshow(line_img, 'gray', vmin=np.mean(line_img)*0.9, vmax=np.max(line_img))
    # # ax.set_title(title)
    # ax.set_axis_off()
    # plt.show()
    #
